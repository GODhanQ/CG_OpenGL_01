	...
	if (!mouse_release) {
		if (left_click_mod) {
			...
			for (int i = 0; i < Spread_Rect_dir.size(); ++i) {
				...
				if (Eraser_Rect_Bounds1.first <= Rect_Bounds1.first && Eraser_Rect_Bounds1.second >= Rect_Bounds2.first &&
					Eraser_Rect_Bounds2.first >= Rect_Bounds2.second && Eraser_Rect_Bounds2.second <= Rect_Bounds1.second) {
					Spread_Rect_dir.erase(Spread_Rect_dir.begin() + i);
					auto& Eraser_half_size = std::get<3>(Eraser_Rect);
					Eraser_half_size += 0.01f;
